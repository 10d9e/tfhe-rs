...
impl<G: ByteRandomGenerator> EncryptionRandomGenerator<G> {
    ...
    pub fn fill_slice_with_random_mask<Scalar>(&mut self, output: &mut [Scalar])
    where
...
impl<G: ParallelByteRandomGenerator> EncryptionRandomGenerator<G> {
    ...
    pub(crate) fn par_fork_bsk_to_ggsw<T: UnsignedInteger>(
        ...
        self.par_map_to_encryption_generator(mask_iter, noise_iter)
    }
    ...
    fn par_map_to_encryption_generator(
        &mut self,
